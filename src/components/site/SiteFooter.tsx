...
const footerByLocale: Record<Locale, { description: string; rights: string }> = {
  bg: {
  ...
  return (
    <footer className="border-t border-brand-accent/20 bg-brand-surface/80 py-10">
      <Container className="space-y-4 text-center text-sm text-brand-muted sm:text-left">
        <p className="font-heading text-xl text-brand-accentSoft">{brandName}</p>
        <p>{content.description}</p>
        <p>(c) {new Date().getFullYear()} {brandName}. {content.rights}</p>
      </Container>
    </footer>
  );
}